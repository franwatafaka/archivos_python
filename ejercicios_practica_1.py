...
def ej1():
    ...
    stock.update({'arandelas': 300})
    stock.update({'tornillos': 100})
    stock.update({'tuercas': 150})
...
def ej2():
    ...
    # Comenzar aquí, recuerde el identado dentro de esta funcion
        print('Ingrese el nombre del producto para agregarle stock: ')
        ...
        if articulo == 'FIN':
            ...
            for i in range(len(stock)):
                try:
                    print('test que trae i : ', stock[articulo])
                    if articulo in stock:
                        print('Ingrese el nuevo stock de ', articulo, ' : ')
                        nuevo_stock = int(input())
                        
                        stock[articulo]+= nuevo_stock
                        print('Nuevo stock añadido a: ', articulo)
                        break
                    else:
                        print('Producto ingresado: ', articulo, ' debe ingresar un articulo valido ')
                        break
                except:
                    print('Error de articulo ingresado, ', articulo, ' no existe ')
                    i = len(stock)
                    break
